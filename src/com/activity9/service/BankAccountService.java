...
import com.activity9.exception.AccountNotFoundException;
import com.activity9.exception.InsufficientFundsException;
import com.activity9.exception.InvalidAccountException;
import com.activity9.exception.InvalidTransferException;
import com.activity9.model.BankAccount;
...
public class BankAccountService {
	...
	public void createAccount(String accountNumber, String ownerName, double initialBalance)
		...
		if (accountNumber == null || accountNumber.trim().isEmpty()) {
			...
			// empty"
			logger.warn("Account creation failed: account number is null or empty {}", accountNumber);

			// TODO 4: Throw InvalidAccountException with message "Account number cannot be
			// null or empty"
			throw new InvalidAccountException("Account number is null or empty");
		}
		...
		if (ownerName == null || ownerName.trim().isEmpty()) {
			...
			// empty"
			logger.warn("Account creation failed: owner name is null or empty {}", ownerName);
		...
		if (initialBalance < MINIMUM_INITIAL_BALANCE) {
			...
			throw new InvalidAccountException(
					String.format("Minimum initial Balance: %s, initial balance: %s",
		...
		if (accountExists(accountNumber)) {
			...
			throw new InvalidAccountException("Account number already exist: " + accountNumber);
	...
	public void transfer(String fromAccountNumber, String toAccountNumber, double amount)
		...
		// Parameters: fromAccountNumber, toAccountNumber, amount
				logger.info("Transfer request: {} -> {}, amount: {}", fromAccountNumber);
		// Validate amount
		if (amount <= 0) {
